...
class Parsedown
{
    ...
    protected function lines(array $lines)
    {
        ...
        foreach ($lines as $line)
        {
            ...
            $Line = array('body' => $line, 'indent' => $indent, 'text' 
=> $text);
            ...
            {
                $Block = 
$this->{'block'.$CurrentBlock['type'].'Continue'}($Line, $CurrentBlock);
...
$CurrentBlock);
            ...
            if (isset($CurrentBlock) and ! isset($CurrentBlock['type']) 
...
isset($Block['interrupted']))
    ...
    protected function blockComment($Line)
    {
        ...
        if (isset($Line['text'][3]) and $Line['text'][3] === '-' and 
$Line['text'][2] === '-' and $Line['text'][1] === '!')
        {
            $Block = array(
                'markup' => $Line['body'],
            );
    ...
    protected function blockFencedCodeContinue($Line, $Block)
    {
        ...
        {
            $Block['element']['text']['text'] = 
substr($Block['element']['text']['text'], 1);
...
preg_match('/^'.$Block['pattern'].'(?:[ ]+(.*)|$)/', $Line['text'], 
$matches))
        {
            if (isset($Block['interrupted']))
            {
...
isset($Block['interrupted']))
        ...
        {
            $Block['element']['name'] = $Line['text'][0] === '=' ? 'h1' 
: 'h2';
...
]*'.$this->regexHtmlAttribute.')*[ ]*>/i', $Line['text'])) # open
        {
            $Block['depth'] ++;
        }
...
isset($Block['interrupted']))
        ...
        if (strpos($Block['element']['text'], '|') !== false and 
chop($Line['text'], ' -:|') === '')
        {
            $alignments = array();
...
$markerPosition)
                ...
                # compile the inline
                $markup .= isset($Inline['markup']) ? $Inline['markup'] 
...
$Inline['extent']);
...
]*(?<!'.$marker.')\1(?!'.$marker.')/s', $Excerpt['text'], $matches))
        {
            $text = $matches[2];
            $text = htmlspecialchars($text, ENT_NOQUOTES, 'UTF-8');
            $text = preg_replace("/[ ]*\n/", ' ', $text);
    ...
    {
        if (strpos($Excerpt['text'], '>') !== false and 
...
$Link['element']['attributes'];
    ...
    protected function inlineLink($Excerpt)
    {
...
]+("[^"]*"|\'[^\']*\'))?[)]/', $remainder, $matches))
        {
            ...
            if (isset($matches[2]))
            {
                $Element['attributes']['title'] = substr($matches[2], 1, 
- 1);
            }
...
array('&amp;', '&lt;'), $Element['attributes']['href']);
    ...
    {
        if ($this->markupEscaped or strpos($Excerpt['text'], '>') === 
false)
        {
            return;
        }

        if ($Excerpt['text'][1] === '/' and preg_match('/^<\/\w*[ 
...
preg_match('/^<!---?[^>-](?:-?[^-])*-->/s', $Excerpt['text'], $matches))
        ...
        if ($Excerpt['text'][1] !== ' ' and preg_match('/^<\w*(?:[ 
]*'.$this->regexHtmlAttribute.')*[ ]*\/?>/s', $Excerpt['text'], 
$matches))
        {
            return array(
                'markup' => $matches[0],
    ...
    {
        if ($Excerpt['text'][0] === '&' and ! preg_match('/^&#?\w+;/', 
$Excerpt['text']))
        {
            return array(
                'markup' => '&amp;',
...
$text);
    ...
    protected function element(array $Element)
    {
        ...
        if (isset($Element['text']))
        {
            ...
            {
                $markup .= 
$this->{$Element['handler']}($Element['text']);
            }
            else
            {
                $markup .= $Element['text'];
